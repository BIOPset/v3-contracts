...
contract("BinaryOptions", (accounts) => {
  ...
  it("stake in BinaryOptions", () => {
    return BinaryOptions.deployed().then(async function (bo) {
      ...
      var pbb = await bo.getPoolBalanceBonus(accounts[2])
  ...
  it("makes a call bet", () => {
    return BinaryOptions.deployed().then(async function (bo) {
      ...
      var pendingClaims = await bo.getPendingClaims(accounts[2]);
      var betRewardBase = await bo.reward();
  ...
  it('testing rewards', () => {
    return BinaryOptions.deployed().then(async function (bo) {
      ...
      var defaultpp = await bo.defaultPriceProvider();
      await bo.bet(1, defaultpp, oneHour, {from: accounts[3], value: toWei(1)});
...
console.log("got here");
      var stakinBonus = await bo.getCombinedStakingBonus(accounts[3]);
      ...
      var stakinBonus2 = await bo.getCombinedStakingBonus(accounts[3]);
