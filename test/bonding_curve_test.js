var BIOPTokenV3 = artifacts.require("BIOPTokenV3");
...
contract("Bonding curve", (accounts) => {
  it("exists", () => {
    return BIOPTokenV3.deployed().then(async function (instance) {
      assert.equal(
        typeof instance,
        "object",
  ...
  it("buy from curve", () => {
    return BIOPTokenV3.deployed().then(async function (bp) {
      
      var balance1 = await bp.balanceOf(accounts[4]);
      ...
      var balance2 = await bp.balanceOf(accounts[4]);
      await bp.buy({ from: accounts[4], value: toWei(1) });
      
      var balance3 = await bp.balanceOf(accounts[4]);
 ...
 /*  describe("Curve integral calulations", async () => {
  ...
  it("sell to curve", () => {
    return BIOPTokenV3.deployed().then(async function (bp) {
      ...
      console.log(`contract balance b4 sale: ${web3.utils.fromWei(cBalance, "ether")}
      `);
      var bv31 = await bp.balanceOf(accounts[4]);
      var recieved1 = await bp.sell(Math.floor(sold1/10), { from: accounts[4] });
      var cBalance2 = await web3.eth.getBalance(bp.address);
      console.log(`contract balance af sale: ${web3.utils.fromWei(cBalance2, "ether")}
      `);
      var cb2 = await web3.eth.getBalance(bp.address);
      var b1 = await web3.eth.getBalance(accounts[4]);
      var bv32 = await bp.balanceOf(accounts[4]);
      console.log(`rpci
      ...
      `);
      var recieved2 = await bp.sell(Math.floor(sold1/10), { from: accounts[4] });
